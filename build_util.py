...
class Dev:
    ...
    def get_sources(self, source_path, source_glob):
        return list(
            map(lambda x: self.get_build_path(source_path) + x, glob.glob(source_glob))
        )
    ...
    def prepare_build(
        ...
        shared_precompiled_header=None,
    ):
    ...
    def CheckPKGConfig(self, context, version):
        ...
        ret = context.TryAction("pkg-config --atleast-pkgconfig-version=%s" % version)[
            0
        ]
        context.Result(ret)
        return ret
    ...
    def CheckLuaPKG(self, context, name, version):
        ...
        res = context.TryAction(
            '$lua -l %s -e "os.exit"(("%s._VERSION == \'%s\'")" and 0 or 1")'
            % (name, name, version)
        )[0]
        context.Result(res)
        return res
