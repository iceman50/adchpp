...
def handleCommand(core, filter, callback):
    ...
    return cm.signalReceive().connect(
        lambda client, cmd, ok: receiveHandler(client, cmd, ok, filter, callback)
    )


def dump(c, code, msg):
    answer = a.AdcCommand(
        a.AdcCommand.CMD_STA, a.AdcCommand.TYPE_INFO, a.AdcCommand.HUB_SID
    )
...
class InfVerifier(object):
    ...
    def validateParam(self, c, cmd, params):
        if len(cmd.getParameters()) < len(params):
            return self.failed(
                c,
                a.AdcCommand.ERROR_PROTOCOL_GENERIC,
                "Too few parameters for " + cmd.getCommand(),
            )
        ...
        for i, param in enumerate(params):
            if not param.match(cmd.getParam(i)):
                return self.failed(
                    c,
                    a.AdcCommand.ERROR_PROTOCOL_GENERIC,
                    cmd.getParam(i) + " doesn't match " + param,
                )
        return True
...
class PasswordHandler(object):
    ...
    def onINF(self, e, cmd, ok):
        ...
        if not foundn or not foundc:
            return self.failed(
                c, a.AdcCommand.ERROR_PROTOCOL_GENERIC, "No valid nick/CID supplied"
            )
    ...
    def onPAS(self, e, cmd, ok):
        ...
        if c.getState() != a.Client.STATE_VERIFY:
            return self.failed(
                c, a.AdcCommand.ERROR_PROTOCOL_GENERIC, "Not in VERIFY state"
            )
        ...
        if not salt:
            return self.failed(
                c, a.AdcCommand.ERROR_PROTOCOL_GENERIC, "You didn't get any salt?"
            )
